...
func (r *MessageRepository) FindConversations(userID uint) ([]models.Message, error) {
	...
	err := r.db.Preload("Sender").Preload("Receiver").Preload("Product").Where("sender_id = ? OR receiver_id = ?", userID, userID).
...
func (r *MessageRepository) FindMessage(userID uint, otherUserID uint) ([]models.Message, error) {
	var messages []models.Message
	err := r.db.Preload("Sender").Preload("Receiver").Preload("Product").
		Where(
